...
function ServiceDialog({ service, onClose }: { service: Service; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!dialog) return;
    dialog.showModal();
    window.history.replaceState(null, "", `/services?service=${service.slug}`);
...
export function ServiceExplorer() {
  ...
  return <><ServiceGroup label="Personal & Enterprise Finance" items={finance} /><ServiceGroup label="Human Resources Solutions" items={hr} />{initialService && <ServiceDialog service={initialService} onClose={() => setInitialService(null)} />}</>;
}
