...
export default async function ContactPage({ searchParams }: { searchParams: Promise<{ error?: string }> }){const params=await searchParams;const emailConfigured=Boolean(process.env.MAILGUN_API_KEY);const formError=params.error==="send"?"We couldn't send your inquiry right now. Please try again or call +1 727-223-5335.":params.error==="invalid"?"Please check the required fields and submit the form again.":"";return <main><SiteHeader /><PageHero eyebrow="Contact Cummins Worldwide" title="Let's build what" accent="comes next." />
  <section className="contact-form-section" id="contact-form">
    ...
    <form className="inquiry-form" action={emailConfigured?"/api/contact":"https://formsubmit.co/kristopher@agmagency.com"} method="POST">
      {!emailConfigured&&<><input type="hidden" name="_subject" value="New Cummins Worldwide website inquiry" /><input type="hidden" name="_next" value="https://cummins-worldwide-navy.vercel.app/thank-you" /><input type="hidden" name="_template" value="table" /></>}
      ...
      <p className="form-note">Your submission will be sent securely to our team.{emailConfigured&&" We'll also email you a confirmation for your records."}</p>
    </form>
  </section>
