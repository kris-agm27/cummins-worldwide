...
export default async function ContactPage({ searchParams }: { searchParams: Promise<{ status?: string }> }){const { status } = await searchParams; return <main><SiteHeader /><PageHero eyebrow="Contact Cummins Worldwide" title="Let's build what" accent="comes next." />
  <section className="contact-form-section" id="contact-form">
    ...
    <form className="inquiry-form" action="/api/contact" method="POST">
      ...
      {status === "email-error" && <p className="form-note" role="alert">We couldn&apos;t send your inquiry right now. Please try again or email admin@agmagency.com.</p>}
      {status === "invalid" && <p className="form-note" role="alert">Please check the required fields and submit the form again.</p>}
      ...
      <p className="form-note">After submitting, you&apos;ll receive an automatic confirmation and our team will be notified.</p>
    </form>
  </section>
